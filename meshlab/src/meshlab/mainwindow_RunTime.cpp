...
void MainWindow::resetLighting()
{
    // Reset lighting
    GLA()->trackball_light.Reset();
    // Reset colors
    vcg::Point4f baseLightAmbientColor = vcg::Point4f(32, 32, 32, 255);
    vcg::Point4f baseLightDiffuseColor = vcg::Point4f(204,204,204, 255);
    vcg::Point4f baseLightSpecularColor = vcg::Point4f(255,255,255, 255);

    //baseLightAmbientColorParam:  QColor( 32, 32, 32),"MeshLab Base Light Ambient Color","MeshLab GLarea's BackGround Color(bottom corner)"));
    //baseLightDiffuseColorParam:  QColor(204,204,204),"MeshLab Base Light Diffuse Color","MeshLab GLarea's BackGround Color(top corner)"));
    //baseLightSpecularColorParam:  QColor(255,255,255),"MeshLab Base Light Specular Color","MeshLab GLarea's BackGround Color(bottom corner)"));

    //GLA()->glas.baseLightAmbientColor.Import(baseLightAmbientColor);
    //GLA()->glas.baseLightDiffuseColor.Import(baseLightDiffuseColor);
    //GLA()->glas.baseLightSpecularColor.Import(baseLightSpecularColor);
}

void MainWindow::saveLighting()
{
    // Saves lighting to disk
    //GLA()->trackball_light.Reset();

    char lightMatrix[200];
    GLA()->trackball_light.ToAscii(lightMatrix);

    // Write transform matrix to home directory
    QString exportFileName, exportFullPath;
    exportFileName = "light.txt";
    FILE * o = fopen(exportFileName.toAscii(),"wb");
    // Write light 3x3 matrix
    fprintf(o,"light-matrix\n");
    fprintf(o, "%s\n",lightMatrix);
    // Write color information

    vcg::Color4b ambient = GLA()->glas.baseLightAmbientColor;
    vcg::Color4b diffuse = GLA()->glas.baseLightDiffuseColor;
    vcg::Color4b specular = GLA()->glas.baseLightSpecularColor;

    //fprintf(o, "%i, %i, %i, %i\n", vcg::Color4b(0,0,0,255));
    //fprintf(o, "%c, %c, %c\n", diffuse.Red, diffuse.Green, diffuse.Blue);
    //fprintf(o, "%c, %c, %c\n", specular.Red, specular.Green, specular.Blue);
    fclose(o);
}

void MainWindow::loadLighting()
{
    // Loads lighting from disk
    QString exportFileName, exportFullPath;
    exportFileName = "light.txt";
    char token[128], matrixToken[] = "light-matrix";
    char lightMatrix[200];

    vcg::Color4b ambient;
    vcg::Color4b diffuse;
    vcg::Color4b specular;

    FILE *fp=fopen(exportFileName.toAscii(),"rb");
    if(fp!=NULL)
    {
        while (fscanf(fp, "%s\n", token) != EOF)
        {
            if (strcmp(token, matrixToken) == 0)
            {
                // Read matrix
                if (!fscanf(fp,"%s\n", &lightMatrix)) return;
    ...
    // Apply values
    GLA()->trackball_light.SetFromAscii(lightMatrix);
    //GLA()->glas.baseLightAmbientColor.Import(ambient);
}
